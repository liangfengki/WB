...
# 确保 .env 中的 ADMIN_USERNAME / ADMIN_PASSWORD 在模块被直接导入时也能生效。
# 若已经在别处调用过 load_dotenv()，此处是 no-op；dotenv 是可选依赖，缺失时静默跳过。
try:
...
# 系统配置 key 约束：^[a-z_][a-z0-9_]*$ 且长度 ≤ 64
_SETTING_KEY_RE = re.compile(r"^[a-z_][a-z0-9_]*$")
_SETTING_KEY_MAX_LEN = 64
# value 序列化后体积上限：64 KB
_SETTING_VALUE_MAX_BYTES = 64 * 1024
...
class Database:
    ...
    def _init_db(self):
        ...
        with self._cursor() as cur:
            ...
            # 写入 contact_qr 初始种子 (幂等: INSERT OR IGNORE)
            cur.execute(
                """INSERT OR IGNORE INTO system_settings (key, value_json)
                   VALUES (?, ?)""",
                ("contact_qr", '{"path":"IMG_0128.PNG"}'),
            )
            ...
            # 初始化管理员账号：凭据仅通过环境变量传入，代码里不硬编码。
            # 需要在 .env 中配置 ADMIN_USERNAME / ADMIN_PASSWORD。
            # 行为：
            #   1. 启动时若目标账号在 admins 表中不存在 → 用 env 中的密码创建
            #   2. 清理历史遗留的 'admin' 默认账号（仅当它不是当前 env 中的账号时）
            # 已存在的目标账号不会被覆盖，允许管理员后续在后台改密。
            ...
            if env_username and env_password:
                ...
                if cur.fetchone() is None:
                    pwd_hash = hashlib.sha256(env_password.encode()).hexdigest()
                    cur.execute(
                        "INSERT INTO admins (username, password_hash) VALUES (?, ?)",
                        (env_username, pwd_hash),
                    )
                # 清理历史默认账号（避免老的 admin/admin123 继续存在）
                if env_username != "admin":
                    cur.execute(
    ...
    def list_licenses(
        ...
        - ``keyword`` 非空时按 ``code`` 或 ``remark`` 做大小写不敏感的模糊匹配。
        """
        conn = self._get_conn()
        conditions = []
        params: List[Any] = []
        ...
        if keyword:
            ...
            if kw:
                conditions.append("(code LIKE ? OR COALESCE(remark, '') LIKE ?)")
                like = f"%{kw}%"
    ...
    def extend_license(
        self, code: str, extra_days: int
    ) -> Optional[Dict[str, Any]]:
        """延长授权码有效期

        - 若 ``code`` 不存在或 ``type == 'lifetime'``，返回 ``None``，且不修改任何数据。
        - 否则 ``base = max(expires_at, now())``（若 ``expires_at`` 为 ``NULL``
          则以 ``now()`` 为基础），新的 ``expires_at = base + extra_days``，
          同时把 ``status`` 置为 ``'active'``。
        - 返回更新后的整行 dict。

        ``extra_days`` 必须可解析为整数且 ``>= 1``，否则抛 ``ValueError``。
        上层路由（Req 10.6）会再做 ``[1, 3650]`` 的上界校验，这里仅做 DB 层防御。
        """
        # 防御性类型校验：不合法的 extra_days 直接抛 ValueError
        try:
            extra_days_int = int(extra_days)
        except (TypeError, ValueError) as e:
            raise ValueError(f"extra_days 必须是整数: {extra_days!r}") from e
        # 避免 bool 被静默当作 int（True -> 1, False -> 0）
        if isinstance(extra_days, bool):
            raise ValueError("extra_days 必须是整数而不是布尔值")
        if extra_days_int < 1:
            raise ValueError("extra_days 必须 >= 1")

        fmt = "%Y-%m-%d %H:%M:%S"
        with self._cursor() as cur:
            cur.execute(
                "SELECT type, expires_at FROM license_keys WHERE code = ?",
                (code,),
            )
            row = cur.fetchone()
            if row is None:
            ...
            if expires_at_str:
                try:
                    current_expires = datetime.strptime(expires_at_str, fmt)
                except ValueError:
                    # DB 中的 expires_at 格式异常时退化为以 now 为基准，
                    # 避免因历史脏数据阻塞管理员的延期操作
                    current_expires = now
            ...
            new_expires = base + timedelta(days=extra_days_int)
            new_expires_str = new_expires.strftime(fmt)

            cur.execute(
                "UPDATE license_keys SET expires_at = ?, status = 'active' "
                "WHERE code = ?",
                (new_expires_str, code),
            )

        return self.get_license(code)
    ...
    def get_usage_logs(
        ...
        可选过滤：
          - ``license_code``：精确匹配 ``license_code``
          - ``ip_address``：精确匹配 ``ip_address``
          - ``action``：精确匹配 ``action``
          - ``date_from`` / ``date_to``：``created_at`` 的闭区间范围（字符串比较，
            调用方传入形如 ``'YYYY-MM-DD'`` 或 ``'YYYY-MM-DD HH:MM:SS'`` 均可）
        """
        conn = self._get_conn()
        conditions = []
        params: List[Any] = []
    ...
    @staticmethod
    def _validate_setting_key(key: Any) -> str:
        """校验 system_settings 的 key；违规抛 ValueError（不让 sqlite3.IntegrityError 冒出）"""
        if not isinstance(key, str):
            raise ValueError("setting key 必须是字符串")
        if len(key) > _SETTING_KEY_MAX_LEN:
            raise ValueError(
                f"setting key 长度不能超过 {_SETTING_KEY_MAX_LEN} 字符"
            )
        if not _SETTING_KEY_RE.match(key):
            raise ValueError(
                "setting key 必须匹配 ^[a-z_][a-z0-9_]*$"
            )
        return key

    @staticmethod
    def _serialize_setting_value(value: Any) -> str:
        """把 value 序列化为 JSON 文本；校验序列化后体积 ≤ 64 KB。"""
        try:
            payload = json.dumps(value, ensure_ascii=False)
        except (TypeError, ValueError) as e:
            raise ValueError(f"setting value 无法 JSON 序列化: {e}") from e
        # 按 UTF-8 字节计量，确保非 ASCII 字符也能严格约束在 64 KB 内
        if len(payload.encode("utf-8")) > _SETTING_VALUE_MAX_BYTES:
            raise ValueError(
                f"setting value 序列化后不能超过 {_SETTING_VALUE_MAX_BYTES} 字节"
            )
        return payload

    def get_setting(self, key: str) -> Optional[Dict[str, Any]]:
        """读取 system_settings 中的 KV 配置。

        Args:
            key: 配置键，必须匹配 ^[a-z_][a-z0-9_]*$ 且长度 ≤ 64
        Returns:
            {"key": <key>, "value": <反序列化对象>, "updated_at": <str>}；
            key 不存在时返回 None。
        """
        ...
        try:
            ...
            # DB 中写入的内容若无法解析,抛 ValueError 让调用方感知数据异常,
            # 而非泄露 sqlite3 相关异常
            raise ValueError(
                f"system_settings[{key}].value_json 无法解析为 JSON: {e}"
            ) from e
        return {
            "key": row["key"],
            "value": parsed,
    ...
    def set_setting(self, key: str, value: Any) -> None:
        """写入/更新 system_settings 中的 KV 配置。

        - 对不存在的 key 执行 INSERT，对已存在的 key 执行 UPDATE；
        - updated_at 回填为 datetime('now','localtime')；
        - 违反 key / value 约束时抛 ValueError（不向上抛 sqlite3.IntegrityError）。
        """
        self._validate_setting_key(key)
        payload = self._serialize_setting_value(value)
        try:
            with self._cursor() as cur:
                cur.execute(
            ...
            # 兜底：理论上 upsert 不会触发 IntegrityError,
            # 一旦触发也统一转成 ValueError 给上层,避免把 sqlite 细节暴露出去
            raise ValueError(f"写入 system_settings 失败: {e}") from e
